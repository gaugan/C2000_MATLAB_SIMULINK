...
int main(void)
{
  ...
  c2000_flash_init();
  init_board();
  rtmSetErrorStatus(Blinky_on_serial_M, 0);
  Blinky_on_serial_initialize();
  configureTimer0(modelBaseRate, systemClock);
  runModel =
    rtmGetErrorStatus(Blinky_on_serial_M) == (NULL);
  enableTimer0Interrupt();
