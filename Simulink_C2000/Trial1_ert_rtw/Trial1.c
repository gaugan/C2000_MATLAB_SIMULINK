...
void Trial1_step(void)
{
  ...
  /* Gain: '<Root>/Vout_Gain' */
  tmp = floor(Trial1_P.Vout_Gain_Gain * (real_T)Trial1_B.ADCINA3VoutFB);
  ...
  Trial1_B.Vin_Gain = tmp < 0.0 ? (uint16_T)-(int16_T)(uint16_T)-tmp : (uint16_T)
    tmp;
  ...
  /* S-Function (c280xcanxmt): '<Root>/Vout_eCAN_XMIT' */
  {
    ...
    ucTXMsgData[0] = (Trial1_B.Vin_Gain & 0xFF);
    ucTXMsgData[1] = (Trial1_B.Vin_Gain >> 8);
  ...
  /* Gain: '<Root>/Vin_Gain' */
  tmp = floor(Trial1_P.Vin_Gain_Gain * (real_T)Trial1_B.ADCINB3VinFB);
  if (rtIsNaN(tmp) || rtIsInf(tmp)) {
    tmp = 0.0;
  } else {
    tmp = fmod(tmp, 65536.0);
   ...
   *  Gain: '<S1>/Gain'
   */
  rtb_Sum = Trial1_P.Gain_Gain * Trial1_P.Vout_Set_Value +
    Trial1_P.Constant_Value;
