...
void Trial2_step(void)
{
  ...
  /* Gain: '<Root>/Vout_Gain' */
  tmp = floor(Trial2_P.Vout_Gain_Gain * (real_T)Trial2_B.ADCINA3VoutFB);
  ...
  Trial2_B.Vin_Gain = tmp < 0.0 ? (uint16_T)-(int16_T)(uint16_T)-tmp : (uint16_T)
    tmp;

  /* End of Gain: '<Root>/Vout_Gain' */

  /* Sum: '<Root>/Sum' incorporates:
   *  Constant: '<Root>/Vout_Ref'
   *  Gain: '<Root>/Analog Gain'
   */
  rtb_Sum = Trial2_P.Vout_Ref_Value - Trial2_P.AnalogGain_Gain * (real_T)
    Trial2_B.Vin_Gain;

  /* DiscreteIntegrator: '<S1>/Integrator' */
  rtb_Saturate = Trial2_DW.Integrator_DSTATE;

  /* Sum: '<S1>/Sum' incorporates:
   *  Gain: '<S1>/Proportional Gain'
   */
  rtb_Saturate += Trial2_P.PIController_P * rtb_Sum;

  /* Saturate: '<S1>/Saturate' */
  if (rtb_Saturate > Trial2_P.PIController_UpperSaturationLim) {
    rtb_Saturate = Trial2_P.PIController_UpperSaturationLim;
  } else {
    if (rtb_Saturate < Trial2_P.PIController_LowerSaturationLim) {
      rtb_Saturate = Trial2_P.PIController_LowerSaturationLim;
    }
  }
  ...
  {
    EPwm2Regs.CMPA.bit.CMPA = (uint16_T)(rtb_Saturate);
  }
  ...
  rtb_Sum *= Trial2_P.PIController_I;
  ...
  /* Gain: '<Root>/Vin_Gain' */
  tmp = floor(Trial2_P.Vin_Gain_Gain * (real_T)Trial2_B.ADCINB3VinFB);
  if (rtIsNaN(tmp) || rtIsInf(tmp)) {
    tmp = 0.0;
  } else {
    tmp = fmod(tmp, 65536.0);
  ...
  Trial2_DW.Integrator_DSTATE += Trial2_P.Integrator_gainval * rtb_Sum;
}
...
void Trial2_initialize(void)
{
  ...
  Trial2_DW.Integrator_DSTATE = Trial2_P.Integrator_IC;
}
