...
DW_Trial2_T Trial2_DW;

/* Real-time model */
RT_MODEL_Trial2_T Trial2_M_;
RT_MODEL_Trial2_T *const Trial2_M = &Trial2_M_;

/* Model step function */
void Trial2_step(void)
{
  real_T rtb_Sum;
  uint16_T qY;
  ...
  if (Trial2_P.Constant_Value > 0.0) {
    /* S-Function (c2802xadc): '<S1>/ADCINA3 - Vout F//B' */
    {
      /*  Internal Reference Voltage : Fixed scale 0 to 3.3 V range.  */
      /*  External Reference Voltage : Allowable ranges of VREFHI(ADCINA0) = 3.3 and VREFLO(tied to ground) = 0  */
      Trial2_B.ADCINA3VoutFB = (AdcaResultRegs.ADCRESULT0);
    }

    /* Sum: '<S1>/Sum' incorporates:
     *  Constant: '<S1>/Vout_Ref'
     *  Gain: '<S1>/Analog Gain'
     */
    rtb_Sum = Trial2_P.Vout_Ref_Value - Trial2_P.AnalogGain_Gain * (real_T)
      Trial2_B.ADCINA3VoutFB;

    /* MATLAB Function: '<S1>/Control Loop' */
    /* MATLAB Function 'Subsystem/Control Loop': '<S2>:1' */
    /* '<S2>:1:4' if isempty(result) */
    /* '<S2>:1:7' num=3; */
    /* '<S2>:1:10' while num>0 */
    /* '<S2>:1:11' if(u < 0) */
    if (rtb_Sum < 0.0) {
      /* '<S2>:1:12' result=result+1; */
      qY = Trial2_DW.result + /*MW:OvSatOk*/ 1U;
      if (qY < Trial2_DW.result) {
        qY = MAX_uint16_T;
      }
    ...
    /* '<S2>:1:18' if(result < 140) */
    if (Trial2_DW.result < 140U) {
      /* '<S2>:1:19' result=uint16(140); */
      Trial2_DW.result = 140U;
    } else {
      if (Trial2_DW.result > 200U) {
        /* '<S2>:1:21' elseif(result>200) */
        /* '<S2>:1:22' result=uint16(200); */
        Trial2_DW.result = 200U;
      }
    }

    /* '<S2>:1:24' num=0; */
    /* '<S2>:1:26' y=result; */
    Trial2_B.y = Trial2_DW.result;

    /* End of MATLAB Function: '<S1>/Control Loop' */

    /* S-Function (c2802xpwm): '<S1>/High-Low Side Driver Signal' */

    /*-- Update CMPA value for ePWM2 --*/
    {
      EPwm2Regs.CMPA.bit.CMPA = (uint16_T)(Trial2_B.y);
    }

    /* S-Function (c280xcanxmt): '<S1>/Vout_eCAN_XMIT' */
    {
      ...
      ucTXMsgData[0] = (Trial2_B.y & 0xFF);
      ucTXMsgData[1] = (Trial2_B.y >> 8);
    ...
    /* S-Function (c2802xadc): '<S1>/ADCINB3 - Vin F//B' */
    {
      /*  Internal Reference Voltage : Fixed scale 0 to 3.3 V range.  */
      /*  External Reference Voltage : Allowable ranges of VREFHI(ADCINA0) = 3.3 and VREFLO(tied to ground) = 0  */
      Trial2_B.ADCINB3VinFB = (AdcbResultRegs.ADCRESULT1);
    }

    /* Gain: '<S1>/Vin_Gain' */
    rtb_Sum = floor(Trial2_P.Vin_Gain_Gain * (real_T)Trial2_B.ADCINB3VinFB);
    ...
    Trial2_B.Vin_Gain = rtb_Sum < 0.0 ? (uint16_T)-(int16_T)(uint16_T)-rtb_Sum :
      (uint16_T)rtb_Sum;

    /* End of Gain: '<S1>/Vin_Gain' */

    /* S-Function (c280xcanxmt): '<S1>/Vin_eCAN_XMIT' */
    {
      tCANMsgObject sTXCANMessage;
      sTXCANMessage.ui32MsgID = 12000005;// CAN message ID - use 1
      sTXCANMessage.ui32MsgIDMask = 0; // no mask needed for TX
      unsigned char ucTXMsgData[2];
      ucTXMsgData[0] = (Trial2_B.Vin_Gain & 0xFF);
      ucTXMsgData[1] = (Trial2_B.Vin_Gain >> 8);
      sTXCANMessage.ui32MsgLen = 2;    // size of message
      sTXCANMessage.pucMsgData = ucTXMsgData;
      sTXCANMessage.ui32Flags = MSG_OBJ_TX_INT_ENABLE;// enable interrupt on TX
      sTXCANMessage.ui32Flags |= MSG_OBJ_EXTENDED_ID;
...
void Trial2_initialize(void)
{
  ...
  rtmSetErrorStatus(Trial2_M, (NULL));

  /* block I/O */
  (void) memset(((void *) &Trial2_B), 0,
                sizeof(B_Trial2_T));

  /* states (dwork) */
  (void) memset((void *)&Trial2_DW, 0,
                sizeof(DW_Trial2_T));

  /* Start for Enabled SubSystem: '<Root>/Subsystem' */

  /* Start for S-Function (c2802xadc): '<S1>/ADCINA3 - Vout F//B' */
  InitAdcA();
  config_ADCA_SOC0 ();
  AdcaRegs.ADCSOCFRC1.bit.SOC0 = 1;

  /* Start for S-Function (c2802xpwm): '<S1>/High-Low Side Driver Signal' */
  EALLOW;
  ...
  /*** Initialize ePWM2 modules ***/
  {
    ...
    EPwm2Regs.CMPC = 140;              // Counter Compare C Register
  ...
  /* Start for S-Function (c2802xadc): '<S1>/ADCINB3 - Vin F//B' */
  InitAdcB();
  config_ADCB_SOC1 ();
  AdcbRegs.ADCSOCFRC1.bit.SOC1 = 1;

  /* Start for S-Function (c2802xpwm): '<S1>/Duty control for Active Load ' */
  EALLOW;
  ...
  /* SystemInitialize for MATLAB Function: '<S1>/Control Loop' */
  /* '<S2>:1:5' result = uint16(0); */
  Trial2_DW.result = 0U;
